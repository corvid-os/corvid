...
| 4 | **Dysk i partycjonowanie** | tryb *Auto* (cały dysk, Btrfs + subwolumeny wg `filesystem-btrfs-snapper.md`) albo *Manual* (odsyła do **GNOME Disks**, patrz `apps.md` — nie budujemy własnego partycjonera) | min. rozmiar dysku (np. 20 GB), potwierdzenie utraty danych |
...
| 9 | **Konto użytkownika** | tylko to co niezbędne do utworzenia pierwszego konta: login, hasło, pełna nazwa, czy konto ma być administratorem (grupa `wheel`) | login wg reguł Unix, siła hasła |
...
- ✅ **Nie budujemy własnego partycjonera ani rozbudowanego zarządzania kontem
  użytkownika w instalatorze.** Tam gdzie system (GNOME) ma już gotowe, dobre
  narzędzie — używamy go zamiast pisać/utrzymywać własne:
  - Partycjonowanie ręczne → **GNOME Disks** (krok 4, patrz `apps.md`)
  - Zarządzanie kontem po instalacji (zmiana hasła, avatar, dodawanie kolejnych
    userów, uprawnienia) → **panel "Użytkownicy" w Ustawieniach systemu**
    (`gnome-control-center`), który i tak jest częścią zainstalowanego systemu.
    Instalator (krok 9) tworzy tylko *pierwsze* konto z minimalnym zestawem pól —
    resztę użytkownik robi później we właściwej apce systemowej, która **już
    domyślnie wymaga hasła/autoryzacji (PolicyKit) do każdej zmiany poza
    kosmetycznymi** (np. zmiana avatara nie wymaga hasła, ale zmiana hasła,
    dodanie usera czy nadanie uprawnień administratora — tak)
...
- Odpowiednik panelu "Użytkownicy" pod Hyprland (GNOME Settings nie jest tam natywne) —
  do ustalenia: czy instalujemy `gnome-control-center` niezależnie od DE tylko dla tego panelu,
  czy szukamy lżejszej alternatywy (np. narzędzie CLI + prosty GUI wrapper)
